     ...
     return (
          <div className = "destination" id="body">
                ...
                <main className="main-container grid-container--destination flow">      
                 ...
                 <img className= "planet-image" src={Array[3]?.img} alt="the moon"/>
                    <div className="tabs-list">
                        <DestinationNav/>
                    </div>      
             <article className="destination-info  flow">
                    <span className="uppercase font-Bellefair leading-mid-tight planet--name">{Array[3]?.name}</span>
                    <p className="max-w-80 text-xs font-Roboto font-light [letter-spacing:1px]">{Array[3]?.description}</p>
               <div>
             <div className="destination-meta">
               <div>
                    <h3 className="uppercase text-xs text-white/60">Avg. distance</h3>
                    <p className="uppercase font-Bellefair text-2xl">{Array[3]?.AvgDistance}</p>
               </div>
               <div>
                    <h3 className="uppercase text-xs text-white/60">Est. travel time</h3>
                    <p className="uppercase font-Bellefair text-2xl">{Array[3]?.travelTime}</p>
               </div>
           </div>
              </div>
          </article>

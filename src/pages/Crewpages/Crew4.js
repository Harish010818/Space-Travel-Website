           ...
           <main className="grid-container--crew main-container justify-items-start flow">
                   ...
                   <article className="crew-details flow">
                            <header className="uppercase [letter-spacing:2px] flow">
                                <h2 className="text-blue-500 leading-mid-tight font-extralight font-Bellefair designation">{Array[3]?.desig}</h2>
                                <p className="crew-name font-Bellefair">{Array[3]?.name}</p>
                            </header>
                          <p className="max-w-80 text-xs font-Roboto font-light [letter-spacing:1px]">{Array[3]?.about}</p>
                   </article>             
                      <img className= "crew-image border-b border-white/10" src={Array[3]?.img} alt="crew-astronauts" />
           </main>
       </div>
           )
}
